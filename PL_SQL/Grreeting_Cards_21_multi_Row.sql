...
create or replace PROCEDURE write_greetings IS
  ...
  temp_holiday holidays.holiday_name%TYPE := 'Platzhalter';         -- probably obsolete
  temp_file holidays.holiday_name%TYPE := 'Platzhalter';            -- probably obsolete
  temp_date    holidays.holiday_date%TYPE := to_date(sysdate + 7);  -- probably obsolete
  
  TYPE temp_holy_type IS TABLE OF holidays%ROWTYPE;	-- initialisation of type-variable based on a row-type
  greetings_holy temp_holy_type;
  
  v_output_dir VARCHAR2(50) := 'GREETING_DIR';

BEGIN
    SELECT * BULK COLLECT INTO greetings_holy
        FROM holidays WHERE sysdate + 7 > holiday_date AND holiday_status like 'offen';     
    --#--------------------------------------#
    SELECT * BULK COLLECT INTO greetings_table
        FROM employees;    
    --#--------------------------------------#
    --##### outer loop #######
    FOR h IN greetings_holy.FIRST..greetings_holy.LAST
    LOOP
        temp_holiday    := greetings_holy(h).holiday_name;
        
        temp_date       := greetings_holy(h).holiday_date;
            
        temp_file := REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(temp_holiday,'ß','ss'),'ü','ue'),'ö','oe'),' ','_'),'ä','ae');
        
        file_ID := UTL_FILE.FOPEN(v_output_dir, temp_file||'.csv','W');
        ...
        update holidays set holiday_status = 'abgerufen'
            where holiday_name = greetings_holy(h).holiday_name 
                AND holiday_date = greetings_holy(h).holiday_date;
        COMMIT;
    END LOOP;
    --########## end outer loop ######    
End;
