...
CREATE OR REPLACE PROCEDURE write_greetings IS
  ...
  TYPE temp_table_type IS TABLE OF employees%ROWTYPE;
  greetings_table temp_table_type;
...
END;
  ...
  BEGIN
    DBMS_SCHEDULER.CREATE_JOB (
            job_name => '"ORA21"."CREATE_GREETINGS_FILE"',
            schedule_name => '"ORA21"."GREETINGS_PLAN"',
            job_type => 'STORED_PROCEDURE',
            job_action => 'ORA21.WRITE_GREETINGS',
            number_of_arguments => 0,
            enabled => FALSE,
            auto_drop => FALSE,
            comments => '');  
    DBMS_SCHEDULER.SET_ATTRIBUTE( 
             name => '"ORA21"."CREATE_GREETINGS_FILE"', 
